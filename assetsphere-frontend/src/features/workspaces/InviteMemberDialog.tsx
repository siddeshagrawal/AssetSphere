...
export function InviteMemberDialog({ workspaceId, open, onOpenChange }: { workspaceId: string; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return <Dialog open={open} onOpenChange={(nextOpen) => nextOpen ? onOpenChange(true) : close()}><DialogContent><DialogHeader><DialogTitle>{invitationUrl ? "Invitation created" : "Invite member"}</DialogTitle><DialogDescription>{invitationUrl ? deliveryMessage : "Invite someone by email and assign their workspace role."}</DialogDescription></DialogHeader>{invitationUrl ? <div className="space-y-4"><div className="flex min-w-0 items-start gap-3 rounded-md bg-emerald-500/10 p-3 text-sm text-emerald-700"><Check className="mt-0.5 h-4 w-4 shrink-0" /><span className="min-w-0 break-all">{invite.data?.emailDeliveryStatus === "SENT" ? "Invitation email sent to" : "Invitation created for"} {invite.data?.inviteeEmail}</span></div><div className="space-y-2"><Label htmlFor="invitation-link">Single-use invitation link</Label><div className="flex min-w-0 flex-col gap-2 sm:flex-row"><Input id="invitation-link" className="min-w-0" readOnly value={invitationUrl} /><div className="flex gap-2"><Button type="button" variant="outline" aria-label="Copy invitation link" onClick={copyLink}>{copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}{copied ? "Copied" : "Copy"}</Button><Button asChild type="button" variant="outline" size="icon"><a href={invitationUrl} target="_blank" rel="noreferrer" aria-label="Open invitation link"><ExternalLink className="h-4 w-4" /></a></Button></div></div><p className="break-words text-xs text-muted-foreground">The link expires at {formatBackendDate(invite.data!.expiresAt, "MMM d, yyyy 'at' h:mm a")} and only the invited email can accept it.</p><span className="sr-only" aria-live="polite">{copied ? "Invitation link copied" : ""}</span></div><DialogFooter><Button type="button" onClick={close}>Done</Button></DialogFooter></div> : <form onSubmit={submit} className="space-y-4"><div className="space-y-2"><Label htmlFor="invite-email">Email</Label><Input id="invite-email" type="email" value={email} maxLength={320} required autoComplete="email" placeholder="colleague@example.com" onChange={(event) => setEmail(event.target.value)} /></div><div className="space-y-2"><Label htmlFor="invite-role">Role</Label><select id="invite-role" value={role} onChange={(event) => setRole(event.target.value as WorkspaceRole)} className="flex h-11 w-full rounded-md border border-input bg-background px-3 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring sm:h-9"><option value="ADMIN">Admin</option><option value="MEMBER">Member</option><option value="VIEWER">Viewer</option><option value="AUDITOR">Auditor</option></select></div>{error && <p className="text-sm text-destructive" role="alert">{error}</p>}<DialogFooter><Button type="button" variant="outline" onClick={close}>Cancel</Button><Button type="submit" disabled={!email.trim() || invite.isPending}>{invite.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}Create invitation</Button></DialogFooter></form>}</DialogContent></Dialog>;
}
