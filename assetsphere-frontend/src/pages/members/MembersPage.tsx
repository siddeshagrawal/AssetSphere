...
export function MembersPage() {
  ...
  return <div className="p-6"><div className="mx-auto max-w-5xl">
    ...
    {members.data && members.data.length > 0 && <div className="overflow-hidden rounded-lg border border-border bg-card"><ul className="divide-y divide-border">{members.data.map((member) => {
      ...
      return <li key={member.id} className="flex items-center gap-4 px-4 py-3.5"><div className="flex h-9 w-9 items-center justify-center rounded-full bg-muted text-xs font-semibold text-muted-foreground">{displayName.slice(0, 2).toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{displayName}{isCurrentUser ? " (you)" : ""}</p><p className="truncate text-xs text-muted-foreground">{member.email ?? (isCurrentUser ? session.user.email : "")}</p><p className="mt-0.5 text-xs text-muted-foreground">Joined {formatBackendDate(member.joinedAt)} · {member.status}</p></div>{canManage && member.role !== "OWNER" ? <div className="flex items-center gap-2"><select aria-label={`Role for ${displayName}`} value={member.role} disabled={changeRole.isPending || isCurrentUser} onChange={(event) => changeRole.mutate({ memberId: member.id, request: { role: event.target.value as WorkspaceRole } })} className="h-8 rounded-md border border-input bg-background px-2 text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">{MANAGEABLE_ROLES.map((role) => <option key={role} value={role}>{role.charAt(0) + role.slice(1).toLowerCase()}</option>)}</select>{canRemove && <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" aria-label={`Actions for ${displayName}`}><MoreHorizontal className="h-4 w-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem className="text-destructive focus:text-destructive" onSelect={() => setSelectedMember(member)}><Trash2 className="h-4 w-4" />Remove member</DropdownMenuItem></DropdownMenuContent></DropdownMenu>}</div> : <span className="rounded bg-muted px-2 py-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{member.role}</span>}</li>;
    })}</ul></div>}
