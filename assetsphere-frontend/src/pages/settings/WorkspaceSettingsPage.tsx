...
export function WorkspaceSettingsPage() {
  ...
  const role = session.status === "AUTHENTICATED"
    ? session.workspaces.find((item) => item.id === workspaceId)?.role
    : undefined;
  const canEdit = role === "OWNER" || role === "ADMIN";
  const workspace = useWorkspace(workspaceId);
  const billing = useWorkspaceBilling(role === "OWNER" ? workspaceId : undefined);
  ...
  return <div className="p-6"><div className="max-w-4xl"><PageHeader title="Workspace settings" description="Manage identity, access, billing, and enabled knowledge capabilities." />
    <div className="space-y-5">
      <section className="rounded-xl border border-border bg-card p-5"><SectionTitle title="General" description="Workspace identity visible to members." /><form onSubmit={submit} className="mt-5 space-y-4"><div className="space-y-2"><Label htmlFor="workspace-name">Name</Label><Input id="workspace-name" value={name} maxLength={160} disabled={!canEdit || update.isPending} onChange={(event) => setName(event.target.value)} /></div><div className="space-y-2"><Label htmlFor="workspace-description">Description</Label><textarea id="workspace-description" value={description} maxLength={2_000} rows={4} disabled={!canEdit || update.isPending} onChange={(event) => setDescription(event.target.value)} className="w-full resize-y rounded-md border border-input bg-background px-3 py-2 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50" /></div><div className="flex flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="min-w-0 break-all text-xs text-muted-foreground">Read-only identifier: /{workspace.data?.slug}</p>{canEdit && <Button type="submit" className="self-start sm:self-auto" disabled={!name.trim() || update.isPending}>{update.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}Save changes</Button>}</div></form></section>
      <div className="grid gap-5 md:grid-cols-2"><SettingsLink icon={Users} title="Members & access" description={canEdit ? "Manage roles and create secure single-use invitations." : "View workspace members and roles."} to={`/workspaces/${workspaceId}/members`} action={canEdit ? "Manage members" : "View members"} /><SettingsLink icon={CreditCard} title="Billing" description={role === "OWNER" ? `Current plan: ${billing.data?.plan ?? "Loading…"}` : "Subscription and usage are managed by the workspace owner."} to={`/workspaces/${workspaceId}/billing`} action={role === "OWNER" ? "View plan and usage" : "View billing access"} /></div>
      <section className="rounded-xl border border-border bg-card p-5"><div className="flex gap-3"><span className="rounded-lg bg-primary/10 p-2"><Bot className="h-4 w-4 text-primary" /></span><SectionTitle title="AI & knowledge" description="Capabilities are enforced by your plan and backend configuration." /></div><div className="mt-5 grid gap-3 sm:grid-cols-3"><Capability label="Available AI models" value={models.isLoading ? "Loading…" : String(availableModels.length)} /><Capability label="Image OCR" value={role === "OWNER" ? (billing.data?.entitlements.ocrEnabled ? "Enabled" : "Plan gated") : "Owner-managed"} /><Capability label="Video transcription" value={role === "OWNER" ? (billing.data?.entitlements.videoTranscriptionEnabled ? "Enabled" : "Plan gated") : "Owner-managed"} /></div>{availableModels.length > 0 && <p className="mt-4 text-xs text-muted-foreground">Available: {availableModels.map((model) => model.displayName).join(", ")}. Model choice is validated server-side for each operation.</p>}</section>
